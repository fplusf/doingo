...
import { DayPickerMultipleProps, isDayPickerMultiple } from '../../types/DayPickerMultiple';
...
export type SelectMultipleModifiers = Pick<Modifiers, InternalModifier.Disabled>;
 ...
 */
export const SelectMultipleContext = createContext<SelectMultipleContextValue | undefined>(
  undefined,
);
...
export function SelectMultipleProvider(props: SelectMultipleProviderProps): JSX.Element {
  ...
  return (
    <SelectMultipleProviderInternal initialProps={props.initialProps} children={props.children} />
  );
}
...
export function SelectMultipleProviderInternal({
  ...
  const onDayClick: DayClickEventHandler = (day, activeModifiers, e) => {
    ...
    const isMinSelected = Boolean(activeModifiers.selected && min && selected?.length === min);
    if (isMinSelected) {
      return;
    }

    const isMaxSelected = Boolean(!activeModifiers.selected && max && selected?.length === max);
    if (isMaxSelected) {
      return;
    }
  ...
  return (
    <SelectMultipleContext.Provider value={contextValue}>{children}</SelectMultipleContext.Provider>
  );
}
