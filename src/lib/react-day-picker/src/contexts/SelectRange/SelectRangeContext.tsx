 ...
 */
export const SelectRangeContext = createContext<SelectRangeContextValue | undefined>(undefined);
...
export function SelectRangeProvider(props: SelectRangeProviderProps): JSX.Element {
  ...
  return (
    <SelectRangeProviderInternal initialProps={props.initialProps} children={props.children} />
  );
}
...
export function SelectRangeProviderInternal({
  ...
  if (max) {
    ...
    if (selectedFrom && selectedTo) {
      const selectedCount = differenceInCalendarDays(selectedTo, selectedFrom) + 1;
